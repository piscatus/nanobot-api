...
public class CryptoUtil {
    ...
    /**
     * Swaps the prefix only.
     *
     * <p>Replacing every occurrence of "nano" would also rewrite the address
     * body, whose base32 alphabet includes n, a and o. Four characters from a
     * 32 character alphabet collide about once in a million positions, and an
     * address body offers 57 of them, so roughly one address in twenty thousand
     * came out several characters short and failed its checksum. Such an
     * address cannot receive - a sender's wallet rejects it - so this corrects
     * what the affected users were shown rather than recovering anything.
     */
    static String formatAddress(String nanoAddress, String ticker) {
        if (!ticker.equalsIgnoreCase("BAN")) {
            return nanoAddress;
        }
        int prefixEnd = nanoAddress.indexOf('_');
        return prefixEnd < 0
            ? nanoAddress
            : "ban" + nanoAddress.substring(prefixEnd);
    }
